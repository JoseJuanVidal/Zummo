report 50141 "Adjust Exchange Rates Zummo"
{
    ApplicationArea = Basic, Suite;
    Caption = 'Adjust Exchange Rates Zummo', Comment = 'ESP="Ajustar tipos de cambio Zummo"';
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    {
                        Caption = 'Adjustment Period', Comment = 'ESP="Periodo de ajuste"';
                        field(StartingDate; StartDate)
                        {
                            ...
                            ToolTip = 'Specifies the beginning of the period for which entries are adjusted. This field is usually left blank, but you can enter a date.',
                                Comment = 'ESP="Especifica el comienzo del periodo para el que se ajustan los movimientos. Normalmente, este campo se deja en blanco, pero puede escribir una fecha."';
                        }
                        field(EndingDate; EndDateReq)
                        {
                            ...
                            ToolTip = 'Specifies the last date for which entries are adjusted. This date is usually the same as the posting date in the Posting Date field.',
                                Comment = 'ESP="Especifica la fecha final en la que se ajustan los movimientos. Esta fecha normalmente coincide con la fecha de registro del campo Fecha registro."';
                    ...
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Posting Description', Comment = 'ESP="Texto de registro"';
                        ToolTip = 'Specifies text for the general ledger entries that are created by the batch job. The default text is Exchange Rate Adjmt. of %1 %2, in which %1 is replaced by the currency code and %2 is replaced by the currency amount that is adjusted. For example, Exchange Rate Adjmt. of DEM 38,000.',
                            Comment = 'ESP="Especifica el texto para los movimientos de contabilidad general creados en el trabajo por lotes. El texto predeterminado es Ajuste divisa de %1 %2, en el que %1 se sustituye por el código de la divisa y %2 por el importe de la divisa ajustado. Por ejemplo, Ajuste divisa de DEM 38 000."';
                    }
                    field(PostingDate; PostingDate)
                    {
                        ...
                        ToolTip = 'Specifies the date on which the general ledger entries are posted. This date is usually the same as the ending date in the Ending Date field.',
                            Comment = 'ESP="Especifica la fecha en la que se registran los movimientos de contabilidad general. Esta fecha normalmente coincide con la fecha final del campo Fecha final."';
                    ...
                    field(DocumentNo; PostingDocNo)
                    {
                        ...
                        ToolTip = 'Specifies the document number that will appear on the general ledger entries that are created by the batch job.', Comment = 'ESP="Especifica el número de documento que aparecerá en los movimientos de contabilidad general creados por el trabajo por lotes."';
                    }
                    field(AdjCustVendBank; AdjCustVendBank)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Adjust Customer, Vendor and Bank Accounts', Comment = 'ESP="Ajuste cliente, proveedor y banco"';
                        MultiLine = true;
                        ToolTip = 'Specifies if you want to adjust customer, vendor, and bank accounts for currency fluctuations.', Comment = 'ESP="Especifica si desea ajustar las fluctuaciones de divisa de clientes, proveedores y cuentas bancarias."';
                        visible = false;
                    }
                    field(AdjGLAcc; AdjGLAcc)
                    {
                        ApplicationArea = Suite;
                        Caption = 'Adjust G/L Accounts for Add.-Reporting Currency', Comment = 'ESP="Ajuste Divisa.-Adic."';
                        MultiLine = true;
                        ToolTip = 'Specifies if you want to post in an additional reporting currency and adjust general ledger accounts for currency fluctuations between LCY and the additional reporting currency.',
                            Comment = 'ESP="Especifica si desea registrar en una divisa adicional de informe y ajustar las cuentas de contabilidad general para las fluctuaciones de divisa entre la divisa local y la adicional."';
                    }
                    field(globalBoolSoloBanco; globalBoolSoloBanco)
                    {
                        ApplicationArea = All;
                        MultiLine = true;
                        Caption = 'Adjust only Bank Accounts', comment = 'ESP="Ajuste solo banco"';
                        Visible = false;
                    }
                }
            }
    ...
    var
        globalBoolSoloBanco: Boolean;
        Text000: Label '%1 must be entered.', Comment = 'ESP="Se debe indicar %1."';
        Text001: Label 'Do you want to adjust general ledger entries for currency fluctuations without adjusting customer, vendor and bank ledger entries? This may result in incorrect currency adjustments to payables, receivables and bank accounts.\\ ',
            Comment = 'ESP="¿Desea ajustar los movs. contabilidad para las fluctuaciones de divisa sin ajustar lo movs. de cliente, proveedor y bancos? Esto puede provocar ajustes incorrectos de divisa en pagos, cobros y cuentas bancos.\\ "';
        Text004: Label 'Do you wish to continue?',
            Comment = 'ESP="¿Confirma que desea continuar?"';
        Text005: Label 'The adjustment of exchange rates has been canceled.',
            Comment = 'ESP="Se ha cancelado el proceso de ajuste de divisas."';
        Text006: Label 'Adjusting exchange rates...\\',
            Comment = 'ESP="Ajustando divisas...\\"';
        Text007: Label 'Bank Account    @1@@@@@@@@@@@@@\\',
            Comment = 'ESP="Banco           @1@@@@@@@@@@@@@\\"';
        Text008: Label 'Customer        @2@@@@@@@@@@@@@\',
            Comment = 'ESP="Cliente         @2@@@@@@@@@@@@@\"';
        Text009: Label 'Vendor          @3@@@@@@@@@@@@@\',
            Comment = 'ESP="Proveedor       @3@@@@@@@@@@@@@\"';
        Text010: Label 'Adjustment      #4#############',
            Comment = 'ESP="Ajuste          #4#############"';
        Text011: Label 'No currencies have been found.',
            Comment = 'ESP="No se han encontrado divisas."';
        Text012: Label 'Adjusting VAT Entries...\\',
            Comment = 'ESP="Ajustando movs. IVA...\\"';
        Text013: Label 'VAT Entry    @1@@@@@@@@@@@@@',
            Comment = 'ESP="Mov. IVA        @1@@@@@@@@@@@@@"';
        Text014: Label 'Adjusting general ledger...\\',
            Comment = 'ESP="Ajustando contabilidad...\\"';
        Text015: Label 'G/L Account    @1@@@@@@@@@@@@@',
            Comment = 'ESP="Cuenta          @1@@@@@@@@@@@@@"';
        Text016: Label 'Adjmt. of %1 %2, Ex.Rate Adjust.',
            Comment = 'ESP="Ajuste de %1 %2, ajuste tipo cambio"';
        Text017: Label '%1 on %2 %3 must be %4. When this %2 is used in %5, the exchange rate adjustment is defined in the %6 field in the %7. %2 %3 is used in the %8 field in the %5. ',
            Comment = 'ESP="%1 en %2 %3 debe ser %4. Cuando este %2 se utiliza en %5, el ajuste tipo cambio se define en el campo %6 en el %7. %2 %3 se utiliza en el campo %8 en el %5. "';
        ...
        Text018: Label 'This posting date cannot be entered because it does not occur within the adjustment period. Reenter the posting date.',
            Comment = 'ESP="Esta fecha de registro no es válida porque no tiene lugar dentro del periodo de ajuste. Vuelva a introducir la fecha de registro."';
        FirstEntry: Boolean;
        MaxAdjExchRateBufIndex: Integer;
        RatesAdjustedMsg: Label 'One or more currency exchange rates have been adjusted.',
            Comment = 'ESP="Se ajustaron uno o más tipos de cambio."';
        NothingToAdjustMsg: Label 'There is nothing to adjust.',
            Comment = 'ESP="No hay nada que ajustar."';
